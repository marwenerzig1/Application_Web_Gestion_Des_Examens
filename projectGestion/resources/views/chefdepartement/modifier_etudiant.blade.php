...
<form name="myForm" action="{{route('etudiants.update3', $etudiant->id )}}" method="POST" enctype="multipart/form-data" >
    @csrf
    @method('PATCH')
    <img class="mb-4" src="{{ asset('images/modifier.png') }}"  alt="" width="100" height="100">
    <h1 class="h3 mb-3 fw-normal" style="color: rgb(8, 24, 49)">Modifier Classe</h1> <br>

      <label id="tt1">Etudiant : </label>
      <select type="text" name="nometprenom" class="form-control" id="floatingInput" > 
        <option value="{{$etudiant->nom}} {{$etudiant->prenom}}">{{$etudiant->nom}} {{$etudiant->prenom}}</option>
        @foreach ($compteEtudiant as $compte)
          @if ($compte->mode == "accepte")
              @if( ($compte->nom." ".$compte->prenom != $etudiant->nom." ".$etudiant->prenom ))
